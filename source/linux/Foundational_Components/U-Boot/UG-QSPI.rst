...
:file:`cadence_qspi.c` driver supports standard SPI mode 0 only.
...
OSPI Controller does not support interfacing with non flash SPI slaves.
...
.. note::
    ...
    The sf command is used to access SPI NOR flash, supporting read/write/erase
    and a few other functions. For more information on sf command in U-boot
    please refer to the u-boot documentation:
...
.. ifconfig:: CONFIG_part_variant in ('AM64X')
    ...
    **Flashing Images to OSPI NOR using TFTP server**
    ...
    In this example, use tftp-hpa in Ubuntu as tftp server. Assume bootloader
    names are 'tiboot3.bin', 'tispl.bin', 'u-boot.img'. Verify ethernet
    connection between AM64x and host machine before proceeding.
    ...
    **Flashing images to OSPI NOR using SD card**
    ...
    'tiboot3.bin', 'tispl.bin', 'u-boot.img'. Boot via SD card boot and stop at
    U-boot prompt before procceeding.
    ...
    **OSPI Boot Mode**
    ...
    Please refer to the AM64x TRM Section 4.4 for more information. The
    following command could also be used to change boot mode to OSPI after
    following one of the methods above. Verify to use the "reset" command which
    uses warm reset and not cold reset.
...
.. ifconfig:: CONFIG_part_variant in ('AM62X')
    ...
    **Flashing Images to OSPI NOR using TFTP server**
    ...
    In this example, use tftp-hpa in Ubuntu as tftp server. Assume bootloader
    names are 'tiboot3.bin', 'tispl.bin', 'u-boot.img'. Verify ethernet
    connection between AM62x SK and host machine before proceeding.
    ...
    **OSPI Boot Mode**
    ...
    Please refer to the AM62x TRM Section 5.4 for more information. The
    following command could also be used to change boot mode to OSPI after
    following one of the methods above. Verify to use the "reset" command which
    uses warm reset and not cold reset.
    ...
    **Flashing Images to OSPI NAND using TFTP server**
    ...
    In this example, use tftp-hpa in Ubuntu as tftp server. Assume bootloader
    names are 'tiboot3.bin', 'tispl.bin', 'u-boot.img'. Verify ethernet
    connection between AM62x LP-SK and host machine before proceeding.
    ...
    **Serial NAND Boot Mode**
    ...
    Please refer to the AM62x TRM Section 5.4 for more information. The
    following command could also be used to change boot mode to Serial NAND
    after following one of the methods above. Verify to use the "reset" command
    which uses warm reset and not cold reset.

    .. code-block:: console

      => mw.l 0x43000030 0x00000003; reset
...
.. ifconfig:: CONFIG_part_variant in ('AM62AX')
    ...
    **Flashing Images to OSPI NAND using TFTP server**
    ...
    In this example, use tftp-hpa in Ubuntu as tftp server. Assume bootloader
    names are 'tiboot3.bin', 'tispl.bin', 'u-boot.img'. Verify ethernet
    connection between AM62Ax and host machine before proceeding.
    ...
    4. Change boot mode pins to boot with Serial NAND boot mode and reboot EVM
    ...
    **Serial NAND Boot Mode**
    ...
    Please refer to the AM62Ax TRM Section 5.4 for more information. The
    following command could also be used to change boot mode to Serial NAND
    after following one of the methods above. Verify to use the "reset" command
    which uses warm reset and not cold reset.
...
.. ifconfig:: CONFIG_part_variant in ('AM62PX')
    ...
    **Flashing Images to OSPI NOR using TFTP server**
    ...
    In this example, use tftp-hpa in Ubuntu as tftp server. Assume bootloader
    names are 'tiboot3.bin', 'tispl.bin', 'u-boot.img'. Verify ethernet
    connection between AM62Px and host machine before proceeding.
